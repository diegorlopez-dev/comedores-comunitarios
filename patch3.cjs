...
if (!dirText.includes('hasActivePostulacion')) {
  ...
  const buttonLogic = {userId !== comedor.usuario_id && (
                          <button
  ...
  dirText = dirText.replace(/\{userId !== comedor\.usuario_id && \([\s\S]*?<\/button>\s*\)\}/, buttonLogic);

  fs.writeFileSync('src/pages/DirectorioComedores.tsx', dirText, 'utf8');
  console.log('Patched Directory!');
}